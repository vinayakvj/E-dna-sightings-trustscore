 ...
 * This markdown file has been provided to enable thorough examination of our approach and processes, with a focus on GBIF. For day to day client use a singular .R script file will also be provided that does not contain the same level of commentary, exploration and visualisations.
...
# Filter data to remove invalid Longitudes
data <- data %>% filter(decimalLongitude < 200)
```
...
```{r}
write_csv(data, "../Data/data_GBIF_enriched.csv")
```