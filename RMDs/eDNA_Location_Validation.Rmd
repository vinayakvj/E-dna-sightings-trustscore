...
# Work from a copy of "data" from  Species_DNA_Completeness.Rmd but only include fish/shark species
data_gbif <- data %>% filter(class %in% c("Actinopterygii", "Chondrichthyes"))
...
```
### Understand and scope the available GBIF data
...
```{r}
# Load taxonomy reference file with GBIF taxonKey attached to each species
tax_ref <- read_csv("../Data/GBIF_Taxonomy.csv", show_col_types = FALSE)

# Build a vector of taxonKeys
taxa <- tax_ref$taxonKey %>% unique() %>% na.omit() %>% as.character()
...
abundance_tbl <- tibble(
  ...
  left_join(tax_ref %>% select(taxonKey, matchedName), by = "taxonKey") %>%
...
abundance_tbl <- abundance_tbl %>%
  mutate(
    abundance_class = case_when(
      is.na(gbif_count_AOI)        ~ "unknown",
      gbif_count_AOI == 0          ~ "absent",
...
write_csv(abundance_tbl, "../Data/gbif_species_abundance_summary.csv")
```
