...
# Also take a max value across both
data$Pct_GenusDNA_inDB_ANY <- pmax(data$ProportionYes_12S,data$ProportionYes_16S, na.rm=TRUE)
str(data)
```
...
data$dr_scaled <- scales::rescale(data$dr, to = c(0, 1))

# Rescale log(dr) (diversification rate) to range 0–1
data$dr_logscaled <- scales::rescale(log(data$dr), to = c(0.0, 1))
...
# Validate multiple columns have identical data (only needed if GBIF_Taxonomy has been corrupted by multiple writes and column duplication)
# sum(GBIF_Taxonomy$gbif_count_AOI.obs == GBIF_Taxonomy$gbif_count_AOI.tax) == nrow(GBIF_Taxonomy)
# Deal with potential issues with variable names
# Drop any columns that have been duplicated due to running the Rmd multiple times
data <- data %>% select(-contains("obis_hits_AU"), -contains("gbif_count_AOI"))


gbif_candidates <- c("gbif_count_AOI", "gbif_count_AOI.tax")
gbif_chosen <- intersect(gbif_candidates, names(GBIF_Taxonomy))[1]
if (is.na(gbif_chosen)) stop("Neither gbif_count_AOI nor gbif_count_AOI.tax exists.")

data <- data %>%
  left_join(
    GBIF_Taxonomy %>%
      select(taxonKey, gbif_count_AOI = all_of(gbif_chosen)),
    by = "taxonKey"
  )
...
data$total_obs = data$gbif_count_AOI + data$obis_hits_AU
data$total_obs_logscaled <- scales::rescale(log(data$total_obs), to = c(0, 1))
data$total_obs_logscaled[data$total_obs_logscaled == -Inf] <- 0
...
data$dr_logscaled[data$dr_logscaled == -Inf] <- 0
data$DNA_score <- data$Pct_GenusDNA_inDB_ANY - (data$dr_logscaled * 0.025)

# If any -Inf values slipped through, set to 0
data <- data %>% mutate(across(c(DNA_score, obs_score), ~ ifelse(is.infinite(.x) & .x < 0, 0, .x)))

# Clean DNA_score (force numeric, replace NA and non-numeric with 0)
data <- data %>%
  mutate(DNA_score = suppressWarnings(as.numeric(DNA_score)),
         DNA_score = replace_na(DNA_score, 0))

# Blunt score = geo_evidence_score + dna_score + bonus points based whether it's a rare/common species
...
dna_cols <-     c("TotalSpecies", "Pct_GenusDNA_inDB", "Pct_GenusDNA_inDB_ANY", "YesCount_12S", "YesCount_16S", "ProportionYes_12S", "ProportionYes_16S", "dr", "dr_scaled", "dr_logscaled")
obs_cols <-     c("gbif_count_AOI", "obis_hits_AU", "total_obs", "total_obs_scaled", "total_obs_logscaled")
...
head(data)
write_csv(data_final, "../Data/data_enriched.csv")
```
