...
# Validate multiple columns have identical data (only needed if GBIF_Taxonomy has been corruped by multiple writes and column duplication)
...
data <- data %>%
  left_join(
    GBIF_Taxonomy %>% select(taxonKey, gbif_count_AOI),
    by = c("taxonKey")
  )
