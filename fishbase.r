...
# ------------------ Inputs --------------------
in_path <-"C://Users//rosam//Downloads//all_voyages.csv//all_voyages.csv"   # adjust to your path

# ------------------ Download FishBase parquet (cached) -------------
...
# Restrict to national level Australia
au_df <- country_df %>%
...
process_row <- function(row) {
  row <- as.list(row)
  sp  <- as.character(row$species %||% "")
  lca <- as.character(row$Species_In_LCA %||% "")
  
  if (tolower(str_squish(sp)) != "dropped") {
    # exact species
    ...
    res <- res %>% mutate(AU_summary = ifelse(in_AU, "T", "F"))
    return(res)
  } else {
    # dropped → parse Species_In_LCA list
    cand_list <- str_split(lca, ",")[[1]] %>% str_squish()
    scs <- map_int(cand_list, name_to_speccode)
    vals <- map_lgl(scs, is_in_au)
    summary_str <- paste(ifelse(vals, "T", "F"), collapse = ",")
    tibble::tibble(
      ASV            = row$ASV,
      sample         = row$sample,
